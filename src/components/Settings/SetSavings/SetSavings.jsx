   ...
   return (
       <form onSubmit={submitSavingsHandler}>
           <label>
               ...
               {!expenseBook.totalSavings && <input onChange={savingsHandler} name="totalSavings" value={newSavings.totalSavings} type="text"/>}
               {expenseBook.totalSavings && <input onChange={savingsHandler} disabled={!isEditing} name="totalSavings" value={newSavings.totalSavings} type="text"/>}
            </label>
               {!expenseBook.totalSavings && <button>Add</button>}
               {isEditing && <button onClick={updateHandler}>Update</button>}
               {expenseBook.totalSavings && !isEditing && <button onClick={editHandler}>Edit</button>}
       </form>
